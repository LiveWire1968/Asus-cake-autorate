...
install_dir="/jffs/addons/cake-autorate/"

# Possible performance improvement
export LC_ALL=C

if ! type opkg fping ping mkfifo   &> /dev/null
then
    echo "*** Make sure ENTWARE , then bash iputils-ping fping coreutils-mkfifo are installed! ***"
    exit
fi
...
[[ -d /var/run/cake-autorate ]] && rm -r /var/run/cake-autorate
	...
	ps | grep -v "grep" | grep -w cake-autorate.sh | awk '{print $1}' |xargs kill
exit 
fi
...
export_log_file()
{
	...
	case $export_type in
		...
		default)
			...
			printf -v log_file_export_datetime '%(%F-%H:%M:%S)T'
        		(($debug)) && log_msg "DEBUG" "Exporting log file with default path: /tmp/cake-autorate_$log_file_export_datetime.log"
        		log_file_export_path="/tmp/cake-autorate_$log_file_export_datetime.log"
        		;;
	...
	if (($log_file_export_compress)); then
		if [[ -f /tmp/cake-autorate.log.old ]]; then 
			gzip -c /tmp/cake-autorate.log.old > ${log_file_export_path}.gz
			gzip -c /tmp/log/cake-autorate.log >> ${log_file_export_path}.gz
		else
			gzip -c /tmp/cake-autorate.log > ${log_file_export_path}.gz
		fi
	else
		if [[ -f /tmp/cake-autorate.log.old ]]; then 
			cp /tmp/cake-autorate.log.old ${log_file_export_path}.old
			cp /tmp/cake-autorate.log >> $log_file_export_path
		else
			cp /tmp/cake-autorate.log $log_file_export_path
		fi
	fi
...
maintain_log_file()
{
	...
	[[ -f /tmp/cake-autorate.log ]] && rotate_log_file
